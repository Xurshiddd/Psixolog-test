...
const baseUrl = computed(() =>
    props.channel === 'admin' ? '/admin/requests' : '/psiholog/requests',
);
...
watch(
    ...
    async (v) => {
        ...
        localMessages.value = Array.from(map.values()).sort(
            (a, b) => a.id - b.id,
        );
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="h-[calc(100vh-0px)] w-full p-4">
            <div class="grid h-full grid-cols-1 gap-4 lg:grid-cols-12">
                    ...
                    <div class="h-[calc(100%-92px)] overflow-y-auto p-2">
                        ...
                        >
                            <div class="flex items-center justify-between">
                                <div
                                    class="truncate pr-2 text-sm font-semibold text-gray-900 dark:text-gray-100"
                                >
                                    {{ s.name }}
                                </div>
                                <span
                                    v-if="s.unread_count && s.unread_count > 0"
                                    class="inline-flex shrink-0 items-center justify-center rounded-full bg-red-500 px-2.5 py-0.5 text-xs font-semibold text-white"
                                >
                                    {{ s.unread_count }}
                                </span>
                            </div>
                            <div
                                class="mt-1 truncate text-xs text-gray-600 dark:text-gray-400"
                            >
                                {{ s.email }}
                            </div>
                        </button>
                ...
                <!-- ===================== -->
                <section
                    class="hidden h-full overflow-hidden rounded-2xl border border-gray-200 bg-white lg:col-span-9 lg:flex lg:flex-col dark:border-gray-800 dark:bg-gray-950"
                >
                    <!-- Header -->
                    <div
                ...
                <section
                    v-else
                    class="flex h-full flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white lg:hidden dark:border-gray-800 dark:bg-gray-950"
                >
                    <!-- Header with Back -->
                    <div
                        class="flex shrink-0 items-center justify-between border-b border-gray-200 px-4 py-3 dark:border-gray-800"
                    >
                    ...
                    >
                        <div class="flex items-center gap-3">
                            <input
                                ...
                                :disabled="!activeConversationId || sending"
                                @keydown.enter.prevent="sendMessage"
                                class="h-11 flex-1 shrink-0 rounded-xl border border-gray-200 bg-white px-3 text-sm focus:ring-2 focus:ring-gray-300 focus:outline-none disabled:opacity-60 dark:border-gray-800 dark:bg-gray-950 dark:focus:ring-gray-700"
                                placeholder="Xabar yozing..."
                            />
                            <button
                                class="h-11 rounded-xl border border-gray-200 px-4 text-sm font-semibold hover:bg-gray-50 disabled:opacity-60 dark:border-gray-800 dark:hover:bg-gray-900"

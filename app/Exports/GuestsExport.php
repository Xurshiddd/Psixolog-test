...
class GuestsExport implements FromQuery, WithHeadings, WithMapping, ShouldAutoSize, WithEvents
{
    ...
    {
        $resultIds = $guest->usersTestsResults?->pluck('id')->flip() ?? collect();

        $row = [
            $guest->name ?? '-',
            $guest->phone ?? '-',
            $guest->guest?->desired_position ?? '-',
            $guest->guest?->application_status ?? '-',
        ];
